...
class TestThroughputBenchmarks:
    ...
    def test_sustained_10k_tps_processing(
        ...
        for i in range(0, len(test_transactions), batch_size):
            ...
            if (i // batch_size) % 10 == 0:
                ...
                print(
                    f"Produced {produced_count}/{len(test_transactions)} transactions, "
        ...
        assert (
            actual_production_tps >= min_acceptable_tps
        ...
        assert (
            max_cpu_percent < performance_benchmarks["max_cpu_percent"]
    ...
    def test_fraud_detection_processing_throughput(
        ...
        for i, transaction in enumerate(test_transactions):
            ...
            if (i + 1) % 1000 == 0:
                ...
                print(
                    f"Processed {processed_count}/{len(test_transactions)}, "
        ...
        assert actual_tps >= min_acceptable_tps, f"Processing TPS {actual_tps} below minimum {min_acceptable_tps}"
        ...
        assert (
            p99_latency_ms <= performance_benchmarks["max_latency_ms"] * 2
        ), f"P99 latency {p99_latency_ms}ms exceeds limit"

        assert processing_errors < processed_count * 0.01, f"Too many processing errors: {processing_errors}"
    ...
    def test_concurrent_user_processing_scalability(
        ...
        assert concurrent_tps >= 1000, f"Concurrent TPS {concurrent_tps} is too low"

        assert total_errors < total_transactions * 0.02, f"Too many errors in concurrent processing: {total_errors}"
    ...
    def test_memory_usage_under_sustained_load(
        ...
        assert memory_growth_rate < 10, f"Memory growth rate {memory_growth_rate}MB per batch is too high"
    ...
    def test_redis_connection_pool_performance(self, redis_client, clean_test_environment):
        ...
        assert operations_per_second >= 5000, f"Redis operations/sec {operations_per_second} is too low"

        assert avg_thread_latency <= 10, f"Average Redis latency {avg_thread_latency}ms is too high"
