...
class TestKafkaIntegration:
    ...
    def test_end_to_end_message_flow(
        ...
        for i, transaction in enumerate(test_messages):
            ...
            kafka_producer.produce(topic=topic_name, key=message_key, value=message_value)

            produced_messages.append({"key": message_key, "value": transaction})
    ...
    def test_exactly_once_processing_semantics(self, kafka_config, test_topics):
        ...
        for i in range(50):
            ...
            producer.produce(topic=topic_name, key=f"user_{i % 10}", value=json.dumps(transaction))
