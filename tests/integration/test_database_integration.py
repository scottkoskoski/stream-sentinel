...
class TestPostgreSQLIntegration:
    ...
    def test_schema_creation_with_migration_runner(self):
        ...
        for name, success in results.items():
            assert success, (
                f"Schema migration '{name}' failed: "
                f"{[e for n, e in runner.errors if n == name]}"
            )
    ...
    def test_fraud_alert_insert_and_query(self):
        ...
        alert = FraudAlert(
            ...
            fraud_score=0.85,
            ml_prediction=0.82,
            business_rules_triggered=["high_amount_transaction", "unusual_hour_transaction"],
        ...
        with self.pg.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM fraud_alerts WHERE alert_id = %s",
                    (alert_id,)
                )
                row = cursor.fetchone()
    ...
    def test_fraud_alert_status_update(self):
        ...
        with self.pg.get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT status, investigator_id, resolution_notes "
                    "FROM fraud_alerts WHERE alert_id = %s",
                    (alert_id,)
                )
                row = cursor.fetchone()
...
class TestClickHouseIntegration:
    ...
    def test_schema_creation(self):
        ...
        for name, success in results.items():
            assert success, (
                f"ClickHouse migration '{name}' failed: "
                f"{[e for n, e in runner.errors if n == name]}"
            )
        ...
        for name, ddl in schemas.items():
            ...
            if 'CREATE TABLE' in ddl:
                ...
                if table_body_start > 0 and table_body_end > table_body_start:
                    ...
                    assert 'INDEX idx_' not in table_body, (
                        f"Schema '{name}' still has invalid inline INDEX syntax"
                    )
