...
class TestRunner:
    ...
    def check_infrastructure_status(self) -> Dict[str, bool]:
        ...
        for service, config in services.items():
            try:
                ...
                port_open = any(conn.laddr.port == config["port"] and conn.status == "LISTEN" for conn in connections)
    ...
    def run_test_category(self, category: str, verbose: bool = False, fail_fast: bool = False) -> Dict[str, Any]:
        ...
        cmd.extend(["--color=yes", f"--junitxml={self.results_dir}/{category}_results.xml"])
    ...
    def run_smoke_tests(self) -> bool:
        ...
        cmd = [
            ...
            str(
                self.test_root
                / "unit"
                / "test_feature_engineering.py::TestFeatureEngineering::test_basic_transaction_features"
            ),
            "-v",
            "--tb=short",
        ]
    ...
    def cleanup_test_environment(self):
        ...
        temp_dirs = [self.test_root / "temp", self.test_root / "cache"]
...
Examples:
  ...
  python run_tests.py --performance --verbose    # Performance tests with details
    ...
    parser.add_argument(
        "--category",
        choices=["unit", "integration", "e2e", "performance", "chaos"],
        help="Run specific test category",
    )
    parser.add_argument("--all", action="store_true", help="Run all test categories")
    parser.add_argument("--smoke", action="store_true", help="Run quick smoke tests only")
    parser.add_argument("--regression", action="store_true", help="Run performance regression check")
    ...
    parser.add_argument("--output-dir", default="tests/results", help="Output directory for results")
