...
class ModelExporter:
    ...
    def export_to_onnx(
        ...
        try:
            ...
            validation_result.passed = (
                validation_result.conversion_successful
                and validation_result.accuracy_validated
                and validation_result.performance_validated
    ...
    def _convert_to_onnx(self, model: xgb.XGBClassifier, model_info: Dict[str, Any]) -> onnx.ModelProto:
        ...
        try:
            ...
            if original_feature_names is not None:
                ...
                needs_fixing = any(
                    not name.startswith("f") or not name[1:].isdigit() for name in original_feature_names if name
                )
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix=".onnx", delete=False) as tmp_file:
                ...
                self.logger.info(
                    ...
                    extra={"optimization_level": self.config.optimization_level},
                )
    ...
    def _validate_conversion_accuracy(
        self,
        ...
        try:
            ...
            if not result.accuracy_validated:
                if not accuracy_passed:
                    result.add_error(
                        f"Accuracy tolerance exceeded: {result.max_absolute_error:.2e} > {self.config.accuracy_tolerance:.2e}"
                    )
        ...
        try:
            ...
            if not result.performance_validated:
                if not performance_passed:
                    result.add_error(
                        f"ONNX inference too slow: {result.onnx_inference_time_ms:.2f}ms > {baseline_threshold:.2f}ms"
                    )
