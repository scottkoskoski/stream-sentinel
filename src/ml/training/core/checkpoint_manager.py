        ...
        with self._lock:
            for metadata_file in self.metadata_dir.glob("*.json"):
                try:
                    ...
                    self.logger.warning(
                        "checkpoint.metadata_corrupted",
                        extra={"file": str(metadata_file), "error": str(e)},
                    )
    ...
    def _validate_checkpoint(self, checkpoint: ModelCheckpoint) -> None:
        ...
        if free_space < checkpoint.size_estimate * 2:
            raise CheckpointValidationError(
                f"Insufficient disk space: need {checkpoint.size_estimate * 2}, have {free_space}"
            )
...
class CheckpointManager:
    ...
    def recover_from_failure(self, study_name: str) -> Tuple[Optional[ModelCheckpoint], List[str]]:
        ...
        try:
            ...
            self.logger.error("recovery.failed", extra={"study_name": study_name, "error": str(e)})
    ...
    def _cleanup_failed_checkpoint(self, checkpoint_id: str) -> None:
        ...
        try:
            ...
            self.logger.info("failed_checkpoint_cleaned", extra={"checkpoint_id": checkpoint_id})
        except Exception as e:
            self.logger.warning(
                "cleanup_failed",
                extra={"checkpoint_id": checkpoint_id, "error": str(e)},
