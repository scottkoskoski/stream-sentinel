...
class ProcessedDataset:
    ...
    def __post_init__(self):
        ...
        if self.memory_usage_mb is None:
            self.memory_usage_mb = (self.X.memory_usage(deep=True).sum() + self.y.memory_usage(deep=True)) / (
                1024 * 1024
            )
...
class DataCache:
    ...
    def save_dataset(self, dataset: ProcessedDataset, cache_key: str) -> None:
        ...
        try:
            ...
            self.logger.error("cache_save_failed", extra={"cache_key": cache_key, "error": str(e)})
    ...
    def load_dataset(self, cache_key: str) -> Optional[ProcessedDataset]:
        ...
        try:
            ...
            if not self._validate_cache_integrity(dataset, metadata):
                self.logger.warning("cache_integrity_failed", extra={"cache_key": cache_key})
                return None

            self.logger.info(
                "dataset_loaded_from_cache",
                extra={
                    "cache_key": cache_key,
                    "shape": dataset.shape,
                    "fraud_rate": dataset.fraud_rate,
            ...
            self.logger.error("cache_load_failed", extra={"cache_key": cache_key, "error": str(e)})
            return None

    def _validate_cache_integrity(self, dataset: ProcessedDataset, metadata: Dict[str, Any]) -> bool:
...
class DataProcessor:
    ...
    def load_and_preprocess_data(self, force_refresh: bool = False) -> ProcessedDataset:
        ...
        if not force_refresh and self.enable_caching:
            ...
            if cached_dataset is not None:
                self.logger.info(
                    ...
                    extra={"cache_key": cache_key, "shape": cached_dataset.shape},
                )
                return cached_dataset

        # Process data from scratch
        self.logger.info("processing_data_from_scratch", extra={"force_refresh": force_refresh})
        ...
        try:
            ...
            self.logger.error("data_processing_failed", extra={"error": str(e)})
    ...
    def _load_raw_data(self) -> pd.DataFrame:
        ...
        self.logger.info("loading_transaction_data", extra={"path": str(self.transaction_data_path)})

        data = pd.read_csv(self.transaction_data_path)

        # Load identity data if available
        if (
            self.identity_data_path
            and str(self.identity_data_path).strip()
            and self.identity_data_path.exists()
            and self.identity_data_path.is_file()
        ):
            self.logger.info("loading_identity_data", extra={"path": str(self.identity_data_path)})

            identity_data = pd.read_csv(self.identity_data_path)
            data = data.merge(identity_data, on="TransactionID", how="left")

            self.logger.info(
                "identity_data_merged",
                extra={
    ...
    def _preprocess_data(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        self.logger.info("preprocessing_started", extra={"input_shape": data.shape})
    ...
    def _engineer_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if _FEATURE_ENGINEER_AVAILABLE:
            try:
                ...
                self.logger.info(
                    "feature_engineer.batch_features_added",
                    extra={"new_columns": [c for c in X.columns if c.startswith("feat_")]},
                )
    ...
    def _generate_cache_key(self) -> str:
        ...
        identity_mtime = (
            self.identity_data_path.stat().st_mtime
            if (
                self.identity_data_path
                and str(self.identity_data_path).strip()
                and self.identity_data_path.exists()
