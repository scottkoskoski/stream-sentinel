...
class HyperparameterOptimizer:
    ...
    def __init__(self, config: Dict[str, Any], checkpoint_manager: CheckpointManager):
        ...
        self.logger.info(
            "hyperparameter_optimizer.initialized",
            extra={
    ...
    def resume_study(self, study_name: str) -> Optional[StudyHandle]:
        ...
        try:
            # Load existing study
            study = optuna.load_study(study_name=study_name, storage=self.study_storage)

            if not study.trials:
                self.logger.warning("study.no_trials", extra={"study_name": study_name})
                return None
