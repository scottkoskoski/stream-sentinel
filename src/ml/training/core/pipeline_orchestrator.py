...
try:
    from ml.online_learning.model_registry import (
        ModelRegistry, ModelMetadata, ModelStatus, DeploymentStage
    )
    ...
    try:
        ...
        from src.ml.online_learning.model_registry import (
            ModelRegistry, ModelMetadata, ModelStatus, DeploymentStage
        )
        _MODEL_REGISTRY_AVAILABLE = True
    except ImportError:
            ...
            if _MODEL_REGISTRY_AVAILABLE:
                try:
                    ...
                    reg_metadata = ModelMetadata(
                        ...
                        feature_version=",".join(
                            context.processed_dataset.feature_names[:10]
                        ),
                    )
                    ...
                    if success:
                        ...
                        self.logger.warning(
                            "stage.deployment.registry_registration_failed "
                            "(model saved to filesystem as fallback)"
                        )
