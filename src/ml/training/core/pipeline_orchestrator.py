...
try:
    from ml.online_learning.model_registry import DeploymentStage, ModelMetadata, ModelRegistry, ModelStatus
    ...
    try:
        ...
        from src.ml.online_learning.model_registry import DeploymentStage, ModelMetadata, ModelRegistry, ModelStatus
...
class PipelineStateMachine:
    ...
    VALID_TRANSITIONS = {
        PipelineState.INITIALIZED: [
            PipelineState.DATA_LOADING,
            PipelineState.RECOVERING,
        ],
        PipelineState.DATA_LOADING: [PipelineState.DATA_LOADED, PipelineState.FAILED],
        PipelineState.DATA_LOADED: [
            PipelineState.OPTIMIZATION_STARTING,
            PipelineState.FAILED,
        ],
        PipelineState.OPTIMIZATION_STARTING: [
            PipelineState.OPTIMIZATION_RUNNING,
            PipelineState.FAILED,
        ],
        PipelineState.OPTIMIZATION_RUNNING: [
            PipelineState.OPTIMIZATION_COMPLETED,
            PipelineState.FAILED,
        ],
        PipelineState.OPTIMIZATION_COMPLETED: [
            PipelineState.MODEL_VALIDATION,
            PipelineState.FAILED,
        ],
        PipelineState.MODEL_VALIDATION: [
            PipelineState.MODEL_VALIDATED,
            PipelineState.FAILED,
        ],
        PipelineState.MODEL_VALIDATED: [
            PipelineState.PRODUCTION_DEPLOYMENT,
            PipelineState.FAILED,
        ],
        PipelineState.PRODUCTION_DEPLOYMENT: [
    ...
    @classmethod
    def validate_transition(cls, current_state: PipelineState, new_state: PipelineState) -> bool:
...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        self.logger.info(
            "pipeline_orchestrator.initialized",
            extra={
    ...
    def execute_pipeline(self, model_types: List[str]) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self.logger.info(
                "pipeline.started",
                extra={"pipeline_id": pipeline_id, "model_types": model_types},
            )
    ...
    def _execute_data_processing(self, context: PipelineContext) -> PipelineContext:
        ...
        try:
            ...
            self.logger.info(
                "stage.data_processing.completed",
                extra={
    ...
    def _execute_hyperparameter_optimization(self, context: PipelineContext, model_type: str) -> PipelineContext:
        ...
        try:
            self.logger.info(
                "stage.hyperopt.started",
                extra={"pipeline_id": context.pipeline_id, "model_type": model_type},
            )
            ...
            # Run optimization
            optimization_result = self.hyperopt_optimizer.optimize(study_handle, context.processed_dataset)
    ...
    def _execute_model_validation(self, context: PipelineContext) -> PipelineContext:
        ...
        try:
            self.logger.info("stage.validation.started", extra={"pipeline_id": context.pipeline_id})
            ...
            if best_checkpoint.score < min_threshold:
                raise ValidationError(
                    f"Model {metric_label} below minimum threshold " f"({best_checkpoint.score:.4f} < {min_threshold})"
                )
    ...
    def _execute_production_deployment(self, context: PipelineContext) -> PipelineContext:
        ...
        try:
            self.logger.info("stage.deployment.started", extra={"pipeline_id": context.pipeline_id})
            ...
            # Save production model to filesystem (always -- serves as fallback)
            import pickle

            with open(production_model_path, "wb") as f:
                pickle.dump(production_package, f)

            # Save metadata JSON alongside the pickle
            ...
            # --- Register model in ModelRegistry (if available) ---
            registry_registered = False
            if _MODEL_REGISTRY_AVAILABLE:
                try:
                    registry = ModelRegistry()
                    model_id = f"fraud_model_{checkpoint.model_type}"
                    now_iso = datetime.now().isoformat()

                    reg_metadata = ModelMetadata(
                        model_id=model_id,
                        version="",  # auto-generated by registry
                        name=f"Fraud Detection Model ({checkpoint.model_type})",
                        description=(
                            f"Trained by pipeline {context.pipeline_id} with " f"{metric_label}={checkpoint.score:.4f}"
                        ),
                        ...
                        training_duration_minutes=context.total_duration / 60.0,
                        training_samples=context.processed_dataset.shape[0],
                        status=ModelStatus.STAGING,
                        deployment_stage=DeploymentStage.VALIDATION,
                        training_trigger="pipeline",
                        dataset_version=context.processed_dataset.data_hash,
                        feature_version=",".join(context.processed_dataset.feature_names[:10]),
                    )

                    success = registry.register_model(
                        model=production_package,
                        metadata=reg_metadata,
                    )
                    registry_registered = success

                    if success:
                        self.logger.info(
                            "stage.deployment.registry_registered",
                            extra={
                                "model_id": model_id,
                        ...
                        self.logger.warning(
                            "stage.deployment.registry_registration_failed " "(model saved to filesystem as fallback)"
                        )
                except Exception as reg_exc:
                    self.logger.warning(
                        "stage.deployment.registry_unavailable",
    ...
    def _transition_state(self, context: PipelineContext, new_state: PipelineState) -> PipelineContext:
        ...
        if not self.state_machine.validate_transition(context.current_state, new_state):
            raise InvalidStateTransitionError(f"Invalid transition from {context.current_state} to {new_state}")
    ...
    def _load_pipeline_state(self, pipeline_id: str) -> Optional[PipelineContext]:
        ...
        try:
            ...
            self.logger.error("state.load_failed", extra={"pipeline_id": pipeline_id, "error": str(e)})
            return None
    ...
    def _save_pipeline_results(self, context: PipelineContext) -> Dict[str, Any]:
        ...
        results = {
            ...
            "model_performance": {
                ...
                "checkpoint_id": (
                    context.best_model_checkpoint.checkpoint_id if context.best_model_checkpoint else None
                ),
            },
            "error_history": context.error_history,
    ...
    def _cleanup_pipeline_resources(self, context: PipelineContext) -> None:
        ...
        try:
            ...
            self.logger.info("pipeline.resources_cleaned", extra={"pipeline_id": context.pipeline_id})
        except Exception as e:
            self.logger.warning(
                "pipeline.cleanup_failed",
                extra={"pipeline_id": context.pipeline_id, "error": str(e)},
            )
