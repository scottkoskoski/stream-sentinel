...
class PrometheusBackend(MetricsBackend):
    ...
    def __init__(self, gateway_url: Optional[str] = None, job_name: str = "training_pipeline"):
        ...
        try:
            if gateway_url:
                ...
                self.logger.info(
                    "prometheus_backend.initialized",
                    extra={"gateway_url": gateway_url, "job_name": job_name},
                )
        except ImportError:
            self.logger.warning(
                "prometheus_backend.unavailable",
                extra={"reason": "prometheus_client not installed"},
            )
    ...
    def emit_metric(self, metric: Metric) -> None:
        ...
        try:
            ...
            self.logger.error(
                "prometheus_backend.emit_failed",
                extra={"metric_name": metric.name, "error": str(e)},
            )
    ...
    def emit_batch(self, metrics: List[Metric]) -> None:
        ...
        if self.gateway_url and self.metrics_registry:
            try:
                self.push_to_gateway(self.gateway_url, job=self.job_name, registry=self.registry)
            except Exception as e:
                self.logger.error(
                    "prometheus_backend.push_failed",
                    extra={"error": str(e), "metrics_count": len(metrics)},
                )
...
class FileBackend(MetricsBackend):
    ...
    def __init__(self, file_path: Union[str, Path]):
        ...
        self.logger.info("file_backend.initialized", extra={"file_path": str(self.file_path)})
    ...
    def emit_batch(self, metrics: List[Metric]) -> None:
        ...
        with self._lock:
            try:
                ...
                self.logger.error(
                    "file_backend.batch_emit_failed",
                    extra={"metrics_count": len(metrics), "error": str(e)},
                )
...
class MetricsCollector:
    ...
    def __init__(self, backend: MetricsBackend, buffer_size: int = 1000, flush_interval: int = 30):
        """
        ...
        self.logger.info(
            ...
            extra={"buffer_size": buffer_size, "flush_interval": flush_interval},
        )
    ...
    def _flush_buffer(self) -> None:
        ...
        try:
            ...
            self.logger.debug("metrics.batch_flushed", extra={"batch_size": len(batch)})

        except Exception as e:
            self.logger.error(
                "metrics.flush_failed",
                extra={"error": str(e), "batch_size": len(self.buffer)},
            )
    ...
    def _start_background_flush(self) -> None:
        ...
        if not self.shutdown_event.is_set():
            ...
            self.flush_timer = threading.Timer(self.flush_interval, self._start_background_flush)
            self.flush_timer.start()

    def get_metric_summary(self, metric_name: str, window_seconds: int = 300) -> Optional[MetricSummary]:
        """
        ...
        with self.aggregation_lock:
            ...
            recent_values = [value for value, timestamp in values_with_time if timestamp >= cutoff_time]
            ...
            return MetricSummary(
                ...
                start_time=datetime.fromtimestamp(
                    min(timestamp for _, timestamp in values_with_time if timestamp >= cutoff_time)
                ),
...
class TrainingMetrics:
    ...
    def data_loaded(
        ...
        **tags,
        ...
        self._emit_with_default_tags("ml.data.samples_loaded", samples, tags=tags, metric_type="gauge")
        self._emit_with_default_tags("ml.data.features_count", features, tags=tags, metric_type="gauge")
        self._emit_with_default_tags("ml.data.fraud_rate", fraud_rate, tags=tags, metric_type="gauge")
        self._emit_with_default_tags(
            "ml.data.processing_time_seconds",
            processing_time,
            tags=tags,
            metric_type="timer",
        )
        self._emit_with_default_tags("ml.data.memory_usage_mb", memory_mb, tags=tags, metric_type="gauge")

    # Hyperparameter optimization metrics
    def trial_completed(self, trial_number: int, score: float, duration: float, model_type: str, **tags) -> None:
        """Record hyperparameter trial completion."""
        trial_tags = {"model_type": model_type, **tags}

        self._emit_with_default_tags("ml.hyperopt.trial_completed", 1, tags=trial_tags, metric_type="counter")
        self._emit_with_default_tags("ml.hyperopt.trial_score", score, tags=trial_tags, metric_type="gauge")
        self._emit_with_default_tags(
            "ml.hyperopt.trial_duration_seconds",
            duration,
            tags=trial_tags,
            metric_type="timer",
        )
    ...
    def optimization_completed(
        ...
        **tags,
        ...
        self._emit_with_default_tags("ml.hyperopt.best_score", best_score, tags=opt_tags, metric_type="gauge")
        self._emit_with_default_tags("ml.hyperopt.trials_total", n_trials, tags=opt_tags, metric_type="gauge")
        self._emit_with_default_tags(
            "ml.hyperopt.optimization_time_seconds",
            total_time,
            tags=opt_tags,
            metric_type="timer",
        )
        self._emit_with_default_tags(
            "ml.hyperopt.converged",
    ...
    def model_trained(
        ...
        **tags,
        ...
        self._emit_with_default_tags("ml.model.validation_score", score, tags=model_tags, metric_type="gauge")
        self._emit_with_default_tags(
            "ml.model.training_time_seconds",
            training_time,
            tags=model_tags,
            metric_type="timer",
        )
        self._emit_with_default_tags(
            "ml.model.feature_count",
            feature_count,
            tags=model_tags,
            metric_type="gauge",
        )
        self._emit_with_default_tags(
            "ml.model.training_samples",
            sample_count,
            tags=model_tags,
            metric_type="gauge",
        )
    ...
    def pipeline_started(self, pipeline_id: str, model_types: List[str], **tags) -> None:
        ...
        self._emit_with_default_tags("ml.pipeline.started", 1, tags=pipeline_tags, metric_type="counter")
        self._emit_with_default_tags(
            "ml.pipeline.model_types_count",
            len(model_types),
            tags=pipeline_tags,
            metric_type="gauge",
        )
    ...
    def pipeline_completed(
        ...
        **tags,
        ...
        self._emit_with_default_tags("ml.pipeline.completed", 1, tags=pipeline_tags, metric_type="counter")
        self._emit_with_default_tags(
            "ml.pipeline.total_time_seconds",
            total_time,
            tags=pipeline_tags,
            metric_type="timer",
        )
        ...
        if best_score is not None:
            self._emit_with_default_tags(
                "ml.pipeline.best_score",
                best_score,
                tags=pipeline_tags,
                metric_type="gauge",
            )

    def stage_completed(self, stage_name: str, duration: float, success: bool, **tags) -> None:
        """Record pipeline stage completion."""
        stage_tags = {"stage": stage_name, "success": str(success), **tags}

        self._emit_with_default_tags("ml.stage.completed", 1, tags=stage_tags, metric_type="counter")
        self._emit_with_default_tags("ml.stage.duration_seconds", duration, tags=stage_tags, metric_type="timer")
    ...
    def resource_usage(
        ...
        **tags,
        ...
        self._emit_with_default_tags("ml.resources.memory_mb", memory_mb, tags=tags, metric_type="gauge")
        self._emit_with_default_tags("ml.resources.cpu_percent", cpu_percent, tags=tags, metric_type="gauge")

        if gpu_memory_mb is not None:
            self._emit_with_default_tags(
                "ml.resources.gpu_memory_mb",
                gpu_memory_mb,
                tags=tags,
                metric_type="gauge",
            )
    ...
    def error_occurred(self, component: str, error_type: str, recoverable: bool, **tags) -> None:
        ...
        self._emit_with_default_tags("ml.errors.occurred", 1, tags=error_tags, metric_type="counter")

    def recovery_attempted(self, component: str, attempt_number: int, success: bool, **tags) -> None:
        ...
        self._emit_with_default_tags("ml.recovery.attempted", 1, tags=recovery_tags, metric_type="counter")


def create_metrics_collector(backend_type: str = "file", **backend_kwargs) -> MetricsCollector:
    """
    ...
    if backend_type == "file":
        backend = FileBackend(backend_kwargs.get("file_path", "logs/training/metrics.jsonl"))
