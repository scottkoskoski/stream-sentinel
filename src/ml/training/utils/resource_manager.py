...
class GPUResourceHandle(ResourceHandle):
    ...
    def __init__(self, gpu_id: int, allocated_memory_mb: Optional[float] = None):
        ...
        self.logger.info(
            "gpu_resource.allocated",
            extra={"gpu_id": gpu_id, "allocated_memory_mb": allocated_memory_mb},
        )
        ...
        try:
            ...
            for callback in self.cleanup_callbacks:
                try:
                    ...
                    self.logger.warning(
                        "gpu_cleanup_callback.failed",
                        extra={"gpu_id": self.gpu_id, "error": str(e)},
                    )
...
class MemoryResourceHandle(ResourceHandle):
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            self.logger.error("memory_resource.cleanup_failed", extra={"error": str(e)})
        finally:
            # Mark resource as inactive
            self.is_active = False
...
class GPUResourceManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.gpu_available:
            self.logger.info(
                "gpu_resource_manager.initialized",
                extra={
                    "gpu_available": True,
                    "gpu_id": self.gpu_id,
            ...
            self.logger.warning("gpu_resource_manager.no_gpu", extra={"enable_gpu": self.enable_gpu})
    ...
    def _check_gpu_availability(self) -> bool:
        ...
        try:
            ...
            self.logger.warning("gpu_availability_check.failed", extra={"error": str(e)})
    ...
    def get_gpu_usage(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            self.logger.error("gpu_usage.query_failed", extra={"gpu_id": self.gpu_id, "error": str(e)})
    ...
    def allocate_gpu_resource(self, estimated_memory_mb: Optional[float] = None) -> GPUResourceHandle:
        ...
        with self.resource_lock:
            try:
                ...
                if gpu_usage and estimated_memory_mb:
                    ...
                    if estimated_memory_mb > available_mb * 0.8:  # Leave 20% buffer
                        ...
                        if estimated_memory_mb > available_mb * 0.8:
                            raise ResourceAllocationError(
                                f"Insufficient GPU memory: need {estimated_memory_mb}MB, " f"available {available_mb}MB"
                            )
                ...
                handle.add_cleanup_callback(lambda: self.active_handles.pop(handle.resource_id, None))
                ...
                self.logger.error(
                    "gpu_resource.allocation_failed",
                    extra={"estimated_memory_mb": estimated_memory_mb, "error": str(e)},
                )
    ...
    def cleanup_unused_resources(self) -> int:
        ...
        with self.resource_lock:
            ...
            inactive_handles = [handle for handle in self.active_handles.values() if not handle.is_active]
            ...
            for handle in inactive_handles:
                try:
                    ...
                    self.logger.error(
                        "gpu_cleanup.failed",
                        extra={"resource_id": handle.resource_id, "error": str(e)},
                    )
...
class SystemResourceManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.logger.info(
            "system_resource_manager.initialized",
            extra={
    ...
    def get_current_usage(self) -> ResourceUsage:
        ...
        try:
            ...
            self.logger.error("resource_usage.query_failed", extra={"error": str(e)})
    ...
    def check_resource_availability(
        self, memory_gb: Optional[float] = None, disk_gb: Optional[float] = None
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if memory_gb:
            ...
            if memory_gb > available_memory_gb:
                errors.append(f"Insufficient memory: need {memory_gb}GB, " f"available {available_memory_gb:.1f}GB")

        # Check OOM protection threshold
        if usage.memory_percent > self.oom_threshold * 100:
            errors.append(
                f"Memory usage above OOM threshold: {usage.memory_percent:.1f}% " f"> {self.oom_threshold * 100:.1f}%"
            )

        # Check disk space
        if disk_gb and disk_gb > usage.disk_free_gb:
            errors.append(f"Insufficient disk space: need {disk_gb}GB, " f"available {usage.disk_free_gb:.1f}GB")

        if usage.disk_free_gb < self.min_disk_space_gb:
            errors.append(f"Disk space below minimum: {usage.disk_free_gb:.1f}GB " f"< {self.min_disk_space_gb}GB")
    ...
    def allocate_memory_resource(self, memory_gb: float) -> MemoryResourceHandle:
        ...
        self.logger.info(
            "memory_resource.allocated",
            extra={"resource_id": handle.resource_id, "memory_gb": memory_gb},
        )
    ...
    def cleanup_temp_files(self, temp_dirs: Optional[List[Union[str, Path]]] = None) -> int:
        ...
        for temp_dir in temp_dirs:
            ...
            try:
                ...
                self.logger.warning(
                    "temp_cleanup.failed",
                    extra={"temp_dir": str(temp_path), "error": str(e)},
                )

        if cleaned_count > 0:
            self.logger.info("temp_cleanup.completed", extra={"cleaned_count": cleaned_count})
    ...
    def _start_monitoring(self) -> None:
        ...
        def monitor():
            while not self.shutdown_event.is_set():
                try:
                    ...
                    if usage.memory_percent > self.oom_threshold * 100:
                        self.logger.warning(
                            "resource_monitor.memory_warning",
                            extra={
                    ...
                    self.logger.error("resource_monitor.failed", extra={"error": str(e)})
        ...
        self.logger.info(
            "resource_monitoring.started",
            extra={"monitoring_interval": self.monitoring_interval},
        )
