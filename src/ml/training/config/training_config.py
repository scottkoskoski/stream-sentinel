    ...
    def _validate_resource_availability(self, config: TrainingConfig) -> List[str]:
        ...
        try:
            ...
            if available_memory_gb < config.resources.max_memory_gb * 0.5:
                errors.append(
                    f"Insufficient memory: need {config.resources.max_memory_gb}GB, have {available_memory_gb:.1f}GB"
                )
        except ImportError:
            ...
            for dir_path in [
                ...
                if path.exists():
                    ...
                    if free_space_gb < config.resources.min_disk_space_gb:
                        errors.append(
                            f"Insufficient disk space in {dir_path}: need {config.resources.min_disk_space_gb}GB, have {free_space_gb:.1f}GB"
                        )
        except Exception:
...
def load_training_config(config_path: Optional[str] = None, environment: str = "development") -> TrainingConfig:
    """
...
def _apply_environment_overrides(base_config: Dict[str, Any], environment: str) -> Dict[str, Any]:
    ...
    env_defaults = {
        "development": {
            "data": {"max_features": 50, "enable_caching": True},
            "optimization": {"n_trials": 20, "timeout_seconds": 600},  # 10 minutes
            "monitoring": {"log_level": "DEBUG", "enable_progress_bars": True},
        },
        "staging": {
            "optimization": {"n_trials": 100, "timeout_seconds": 3600},  # 1 hour
            "monitoring": {"log_level": "INFO", "enable_progress_bars": False},
        },
        "production": {
            "optimization": {"n_trials": 500, "timeout_seconds": 14400},  # 4 hours
            "monitoring": {
                "log_level": "WARNING",
