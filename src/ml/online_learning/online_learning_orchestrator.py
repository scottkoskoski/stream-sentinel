...
class OnlineLearningOrchestrator:
    ...
    def _start_background_tasks(self) -> None:
        ...
        health_monitor = threading.Thread(target=self._health_monitoring_loop, name="HealthMonitor", daemon=True)
        health_monitor.start()
        self.background_tasks.append(health_monitor)

        # Metrics collection
        metrics_collector = threading.Thread(target=self._metrics_collection_loop, name="MetricsCollector", daemon=True)
        metrics_collector.start()
        self.background_tasks.append(metrics_collector)

        # Workflow cleanup
        cleanup_worker = threading.Thread(target=self._workflow_cleanup_loop, name="WorkflowCleanup", daemon=True)
                    ...
                    if current_model:
                        ...
                        if success:
                            ...
                            deployed = self.model_registry.deploy_model(
                                metadata.model_id, "staging", traffic_percentage=100.0
                            )
        ...
        try:
            ...
            if drift_alerts:
                ...
                for alert in drift_alerts:
                    self._publish_workflow_event(WorkflowEvent.DRIFT_DETECTED, {"alert": alert.to_dict()})
    ...
    def _run_cleanup_tasks(self) -> None:
        ...
        try:
            ...
            old_workflows = [
                wf for wf in self.workflow_history if datetime.fromisoformat(wf["created_at"]) < cutoff_time
            ]
