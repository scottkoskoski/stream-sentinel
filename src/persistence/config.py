...
class PostgreSQLConfig(DatabaseConfig):
    ...
    def get_sqlalchemy_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pool_pre_ping": True,
            "connect_args": {
                "options": f"-c statement_timeout={self.statement_timeout}" f" -c lock_timeout={self.lock_timeout}"
            },
        }
...
class ClickHouseConfig(DatabaseConfig):
    ...
    def get_connection_string(self) -> str:
        ...
        return f"{protocol}://{self.username}:{self.password}@" f"{self.host}:{self.port}/{self.database}"
