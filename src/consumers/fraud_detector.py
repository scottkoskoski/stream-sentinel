...
class FraudDetector:
    ...
    def publish_fraud_alert(
        ...
        try:
            ...
            # Publish to fraud alerts topic (with tracing headers)
    ...
    def publish_fraud_detection_result(
        ...
        try:
            ...
            detection_result = {
                ...
                "detection_metadata": {
                    "ml_model_version": (
                        "ieee_fraud_model_production_v1.0"
            ...
            # Include correlation ID in the result payload when tracing is active
            if TRACING_AVAILABLE:
                corr_id = TracingContext.current_correlation_id()
                if corr_id:
                    detection_result["correlation_id"] = corr_id

            # Publish to fraud detection results topic for persistence (with tracing)
            if TRACING_AVAILABLE:
                traced_produce(
                    self.producer,
    ...
    def _publish_blocked_transaction(
        ...
        try:
            ...
            # Include correlation ID in the payload when tracing is active
            if TRACING_AVAILABLE:
                corr_id = TracingContext.current_correlation_id()
                if corr_id:
                    blocked_event["correlation_id"] = corr_id

            if TRACING_AVAILABLE:
                traced_produce(
                    self.producer,
                    self.blocked_topic,
                    json.dumps(blocked_event),
                    key=str(user_id),
                    callback=self._delivery_callback,
                )
            else:
    ...
    def _run_single(self) -> None:
        ...
        try:
            while self.running:
                ...
                # Extract or create tracing context from message headers
                tracing_ctx = None
                if TRACING_AVAILABLE:
                    ...
                    if self._validator is not None:
                        ...
                        if not validation.is_valid:
                            ...
                            try:
                                ...
                                self.logger.error(
                                    f"DLQ publish failed: {dlq_err}"
                                )
                            self.consumer.commit(msg)
                            continue
                        if validation.warnings:
                            self.logger.debug(
                                "Transaction validation warnings: %s",
                                validation.warnings,
                            )
