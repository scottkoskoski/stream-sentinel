...
class ValidationResult:
    ...
    def success_rate(self) -> float:
        ...
        successful = sum(1 for result in self.service_results.values() 
                        if result.status == ValidationStatus.CONSISTENT)
...
class CrossServiceValidator:
    ...
    async def validate_transaction_flow(self, 
                                      transaction_ids: List[str],
                                      scenario_id: str,
                                      timeout_seconds: int = 60) -> ValidationResult:
        """
        ...
        try:
            ...
            consistency_checks = await self._perform_consistency_checks(
                transaction_ids, scenario_id, service_results
            )
            
            # Data flow validation
            data_flow_validation = await self._validate_data_flow_integrity(
                transaction_ids, service_results
            )
    ...
    async def _perform_consistency_checks(self, 
                                        transaction_ids: List[str],
                                        scenario_id: str,
                                        service_results: Dict[str, ServiceValidationResult]) -> Dict[str, bool]:
        """Perform cross-service consistency checks."""
        consistency_checks = {}
        
        try:
            # User profile consistency (Redis vs PostgreSQL)
            redis_profiles = service_results.get('redis', ServiceValidationResult('redis', ValidationStatus.ERROR, 0, 0)).details.get('user_profiles_found', 0)
            ...
            clickhouse_transactions = service_results.get('clickhouse', ServiceValidationResult('clickhouse', ValidationStatus.ERROR, 0, 0)).details.get('transaction_records_count', 0)
            expected_transactions = len(transaction_ids)
            
            transaction_consistency = abs(clickhouse_transactions - expected_transactions) <= max(1, expected_transactions * 0.1)
            ...
            postgres_alerts = service_results.get('postgres', ServiceValidationResult('postgres', ValidationStatus.ERROR, 0, 0)).details.get('fraud_alerts_count', 0)
    ...
    async def _validate_data_flow_integrity(self, 
                                          transaction_ids: List[str],
                                          service_results: Dict[str, ServiceValidationResult]) -> Dict[str, Any]:
    ...
    def _determine_overall_status(self, 
                                ...
                                consistency_checks: Dict[str, bool]) -> ValidationStatus:
    ...
    def _calculate_performance_impact(self, service_results: Dict[str, ServiceValidationResult]) -> Dict[str, float]:
        ...
        for service_name, result in service_results.items():
            ...
            overhead_percentage = (result.validation_time_ms / typical_operation_time) * 100 if typical_operation_time > 0 else 0
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        avg_validation_time = sum(result.total_validation_time_ms for result in self.validation_history) / len(self.validation_history)
