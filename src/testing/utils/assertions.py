...
import time
import logging
from typing import Callable, Any, Dict, List, Optional, Tuple, Union, Awaitable
...
class AssertionTimeout(Exception):
    ...
    def __init__(self, assertion_name: str, timeout_seconds: int, attempts: int, last_error: str = ""):
        ...
        message = (
            f"Assertion '{assertion_name}' timed out after {timeout_seconds}s "
            f"({attempts} attempts)"
        )
        if last_error:
...
class EventuallyConsistentAssertions:
    ...
    async def eventually_assert(self,
    ...
    async def assert_cross_service_state(self,
        ...
        return await self.eventually_assert(
            assertion_wrapper,
            assertion_name,
            timeout=timeout,
            context=context
    ...
    async def assert_data_consistency(self,
                                    ...
                                    timeout: Optional[int] = None) -> AssertionResult:
        """
    ...
    async def assert_message_flow(self,
        ...
        result = await self.eventually_assert(
            message_flow_assertion,
            assertion_name,
            timeout=timeout,
            context=context
        )
    ...
    async def assert_performance_sla(self,
                                   ...
                                   sample_count: int = 10) -> AssertionResult:
        """
        ...
        throughput_ok = (min_throughput_ops_per_sec is None or 
                        actual_throughput >= min_throughput_ops_per_sec)
        ...
        if not sla_met:
            ...
            if not throughput_ok:
                error_details.append(f"Throughput SLA failed: {actual_throughput:.1f} < {min_throughput_ops_per_sec:.1f} ops/sec")
    ...
    def get_assertion_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_duration_ms": total_duration,
            "average_attempts_per_assertion": sum(
                result.total_attempts for result in self.assertion_history
