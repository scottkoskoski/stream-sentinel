...
from factories.test_data_factory import TestDataFactory, ScenarioType
...
class TestFraudDetectionPipeline:
    ...
    @pytest.mark.asyncio
    async def test_legitimate_baseline_processing(self, 
                                                test_config,
                                                test_data_factory, 
    ...
    async def test_velocity_attack_detection(self,
        ...
        assert len(attack_user_alerts) >= scenario.expected_alerts, \
    ...
    async def test_mixed_population_realistic_scenario(self,
        ...
        def validate_data_consistency(data: Dict[str, Any]) -> bool:
            ...
            logging.info(f"Data consistency check: profiles={redis_profiles}, alerts={postgres_alerts}, "
        ...
        expected_range = (scenario.expected_fraud_rate * 0.5, scenario.expected_fraud_rate * 2.0)
        
        assert expected_range[0] <= actual_fraud_rate <= expected_range[1], \
    ...
    async def test_service_failure_recovery(self,
        ...
        recovery_result = await eventually_consistent_assertions.eventually_assert(
            ...
            assertion_name="redis_service_recovery", 
            timeout=60
        )
