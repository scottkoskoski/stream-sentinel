...
class UserBehaviorProfile:
    ...
    def generate_transaction_amount(self) -> float:
        ...
        amount = np.random.lognormal(mean=np.log(self.avg_transaction_amount), sigma=sigma)
...
class TestDataFactory:
    ...
    def create_legitimate_baseline_scenario(
        self,
    ...
    def create_velocity_attack_scenario(
        self,
        ...
        total_transactions = int(
            sum(profile.transaction_frequency_per_day * duration_hours / 24 for profile in user_profiles)
        )
    ...
    def create_amount_anomaly_scenario(
        self,
    ...
    def create_mixed_population_scenario(
        self,
        total_users: int = 100,
        duration_hours: float = 24.0,
        ...
        for i in range(fraud_user_count):
            profile = UserBehaviorProfile(
                ...
                transaction_frequency_per_day=high_risk_template["transaction_frequency_per_day"]
    ...
    def generate_transactions_for_scenario(self, scenario: FraudScenario) -> Generator[Dict[str, Any], None, None]:
        ...
        for profile in scenario.user_profiles:
            user_transaction_count = int(profile.transaction_frequency_per_day * scenario.duration_hours / 24)

            for _ in range(user_transaction_count):
                # Schedule transaction at random time within duration
                transaction_time = start_time + timedelta(seconds=random.uniform(0, scenario.duration_hours * 3600))
