...
class ServiceOrchestrator:
    ...
    async def start_services(self, profile: ServiceProfile) -> Dict[str, ServiceInfo]:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully started {len(services_to_start)} services in {total_startup_time:.2f}s"
            )
    ...
    async def _start_single_service(self, service_name: str, compose_file: Path, profile: ServiceProfile):
        ...
        try:
            ...
            self.logger.info(
                f"Service {service_name} started successfully in {service_info.startup_time:.2f}s"
            )
    ...
    async def _wait_for_all_services_healthy(self, services: List[str]):
        ...
        health_check_tasks = [
            self._wait_for_service_healthy(service_name) 
    ...
    def _get_services_for_profile(self, profile: ServiceProfile) -> List[str]:
        ...
        if profile == ServiceProfile.FAST_INTEGRATION:
            ...
            return ["redis", "kafka", "zookeeper", "postgres", "clickhouse", 
                   "fraud_detector", "alert_processor"]
    ...
    def _generate_service_config(self, service_name: str, service_info: ServiceInfo, 
                                profile: ServiceProfile) -> Dict[str, Any]:
    ...
    def get_service_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "service_states": {
                name: info.state.value for name, info in self.services.items()
            },
            "health_check_stats": {
                name: {
